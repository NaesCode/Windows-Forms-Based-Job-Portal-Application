...
namespace Job_Application_Manager
{
    internal class DatabaseSupport
    {
        ...
        internal string? GetUserNameByEmail(string email) //Used in changing passwords
        {
        ...
        internal DataTable? getCompanyData()
        {
            try
            {
                using (OleDbConnection myConn = new OleDbConnection(connectionString))
        ...
        internal void updateCompanyStatus(int companyUserID, string status)
        {
        ...
        internal void IsPostedStatus(int companyUserID, bool status)
        {
        ...
        internal void RetrieveFiles(int companyUserID)
        {
        ...
        internal List<JobPostPanel> GetJobPosts(int hunterID)
        {
                ...
                using (OleDbCommand cmd = new OleDbCommand(query, conn))
                {
                    ...
                    using (OleDbDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            var jobPostPanel = new JobPostPanel(postID, companyName, jobTitle, jobType, location, workMode, startingSalary, vacancy, logo, hunterID)
                            {
        ...
        internal List<ViewApplicants> GetJobPostApplicants(int companyID)
        {
