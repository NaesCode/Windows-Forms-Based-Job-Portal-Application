...
{
    public partial class CompanyEmailingForm : Form
    {
        ...
        private Dictionary<string, object?>? CompanyEmailData;

        private string? recipientEmail;

        private List<string> attachmentPaths = new List<string>();

        public CompanyEmailingForm(int companyID, int hunterID)
        {
            InitializeComponent();
        ...
        {
            CompanyEmailData = dbSupport.GetCompanyEmailData(CompanyID);
            recipientEmail = dbSupport.GetHunterEmail(HunterID);
            JobHunterEmail.Text = recipientEmail;
        }
        ...
        {
            string? SenderEmail = CompanyEmailData?["CompanyEmail"]?.ToString();
            string? AppPassword = CompanyEmailData?["GmailAppPassword"]?.ToString();
            ...
            if (string.IsNullOrEmpty(JobHunterEmail.Text) || string.IsNullOrEmpty(EmailSubject.Text) || EmailBody.Text == "Type your email message here...")
            {
            ...
            {
                message.To.Add(JobHunterEmail.Text);
